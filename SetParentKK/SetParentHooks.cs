...
namespace SetParentKK
{
	public static class SetParentHooks
	{
		...
		public static void CrossFadePrefix(float transitionDuration)
		{
			if(setParentObj != null && setParentObj.setFlag)
			{
				SetParent.CtrlState oldState = setParentObj.currentCtrlstate;
				setParentObj.currentCtrlstate = SetParent.CtrlState.Following;
				setParentObj.isFade = true;
				setParentObj.StartCoroutine(setParentObj.MotionChangeUpdate(setParentObj.femaleSpinePos, setParentObj.femaleBase, transitionDuration, oldState));
			}
